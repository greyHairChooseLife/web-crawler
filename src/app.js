	...
	for(let z=0; z<categoryPool.length; z++){
		if(userAnswer === categoryPool[z].categoryId){
			...
			const consoleMsg = categoryPool[z].numberOfProject === 0 
				...
				: `이 카테고리는 총 ${categoryPool[z].numberOfProject}개 중 ${categoryPool[z].numberOfScraped}개 진행되었습니다. 이어서 진행합니다...(${Math.floor(categoryPool[z].numberOfScraped/categoryPool[z].numberOfProject) * 100}%)`
			...
			try{
				if(categoryPool[z].numberOfProject === 0){
					console.log('\n', new Array(50).join('='), '\n', new Array(50).join('='))
					console.log('\n', new Array(18).join(' '), 'STAGE 1 start...')
					console.log('\n', new Array(50).join('='), '\n', new Array(50).join('='), '\n')
			...
			try{
				if(categoryPool[z].numberOfProject !== targetPool.length){
					if(targetPool.length === 0){
						console.log('\n', new Array(50).join('='), '\n', new Array(50).join('='))
						console.log('\n', new Array(18).join(' '), 'STAGE 2 start...')
						console.log('\n', new Array(50).join('='), '\n', new Array(50).join('='), '\n')
			...
			//////////////////////////////////////////////////////////////////////////////////////////////////////
			console.log('\n', new Array(50).join('='), '\n', new Array(50).join('='))
			console.log('\n', new Array(18).join(' '), 'STAGE 3 start...')
			console.log('\n', new Array(50).join('='), '\n', new Array(50).join('='), '\n')
			...
			let i = 0;
			while(i < targetPool.length) {
				console.time(`time taken`)
				console.log(`[${i}]번째 target을 시작합니다. project(aka. campaign)_state : [${targetPool[i].state}]`);
				...
				if(targetPool[i].state !== 'submitted'){
					if(targetPool[i].foundPage && targetPool[i].foundUpdates && targetPool[i].foundComments && wasTargetVirgin){			//	page, update, comment 세가지 모두 완료시 categoryPool.js 파일도 업데이트(numberOfScraped +1) 해 준다.
						...
						fs.writeFileSync(`./categoryPool.js`, `exports.categoryPool = ${JSON.stringify(categoryPool)}`)
						console.log(`\n./targets/${categoryPool[z].subCategory}.js 파일이 업데이트 되었습니다.\n/categoryPool파일이 업데이트 되었습니다.\n`)
						numberOfScraping++;
					}
				}else{
				...
				console.log(`[${i}]번째 target이 완료되었습니다.`);
			...
			console.log('\n', new Array(110).join('='), '\n', new Array(110).join('='))
			console.log('\n', new Array(18).join(' '), numberOfScraping === 0 ? `카테고리ID: [${userAnswer}]의 모든 프로젝트(캠페인)가 완료되었습니다. 프로그램을 종료합니다...\n` : `\n${numberOfScraping}개의 스크랩을 완료하였습니다. 프로그램을 종료합니다...\n`)
			console.timeEnd('STAGE 3 total time taken')
			console.log('\n', new Array(110).join('='), '\n', new Array(110).join('='), '\n\n')
			
			!isAll && process.exit(1);
		}
	}
...
(
	async () => {
		...
		//	2번째 인자로 all을 넘겨주었다면 해당 메인 카테고리(showCategory.js 참고)에 속한 모든 서브카테고리를 차례로 검색한다.
		if(process.argv[3] === 'all') {
			...
			switch(process.argv[2]) {
				...
				case '149':
					...
					break;
				default:
					console.log('\n없는 분류번호입니다. 프로그램이 종료됩니다...\n')
					process.exit(1)
			}

			//	find subCategories group by selected mainCategory with argv[2]
			const subCategoryArray = categoryPool.reduce((prev, curr) => {
				if(curr.mainCategory === selectedMainCategory) prev.push(curr.categoryId);
				return prev;
			}, [])

			//	iterate app() for all of subCategories group
			for await (const subCategory of subCategoryArray) {
				userAnswer = subCategory;
				await app(true);
			}
