...
const runApp = () => {
	rl.question('카테고리 아이디를 입력 해 주세요: ', async userAnswer => {
		...
		if(categoryPool?.length === undefined || categoryPool?.length < 1){
		...
		for(let z=0; z<categoryPool.length; z++){
			if(userAnswer === categoryPool[z].categoryId){
				isFound = true;
				let consoleMsg;
				if(categoryPool[z].numberOfProject === 0) consoleMsg = '이 카테고리는 모아진 자료가 없습니다. 지금 시작합니다...';
				else consoleMsg = `${categoryPool[z].numberOfProject}개 중 ${categoryPool[z].numberOfScraped}개 진행되었습니다. 이어서 진행합니다...(${Math.floor(categoryPool[z].numberOfScraped/categoryPool[z].numberOfProject)}%)`
				...
				try{
					if(categoryPool[z].numberOfProject === 0){
						...
						console.log(`\n${categoryPool[z].subCategory}.js 파일을 완성했습니다! :>`);
				...
				try{
					if(categoryPool[z].numberOfProject !== targetPool.length){
						if(targetPool.length === 0){
							...
							console.log('두 단계의 조사에도 불충분한 것이 있습니다... 파일 길이 등을 손수 체크 해 보세요.')
				...
				//for(i=0; i<30; i++){
					...
					if(targetPool[i].state !== 'submitted'){
						if(targetPool[i].foundPage && targetPool[i].foundUpdates && targetPool[i].foundComments && wasTargetVirgin){			//	page, update, comment 세가지 모두 완료시 categoryPool.js 파일도 업데이트 해 준다.
		...
		//	유저가 입력한 categoryId를 categoryPool.js에서 찾을 수 없을 때.
		if(!isFound){	
			console.log(`\n없는 카테고리 아이디 입니다.\n\n카테고리 아이디는 '___'에서 확인하세요.\n프로그램 종료는 [Ctrl + c]\n`);
			runApp();
		}
	});
}
