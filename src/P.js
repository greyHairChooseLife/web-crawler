...
async function grepLiveSuccessFailCancel(url, projectState, slugFromTargetData) {
	...
	try {
		...
		const campaignSlug = makeCampaignSlug(url);

		if(fromCampaignGraph === undefined) fromCampaignGraph = await getCampaignData(url, campaignSlug) 
		...
		if(projectState !== 'live') {
			fundingPeriod = await page.$eval('#content-wrap > div.NS_projects__content > section.js-project-content.js-project-description-content.project-content > div > div > div', ele => {
 				ele = ele.querySelector('div.col.col-4.js-rewards-column.max-w62.sticky-rewards > div > div.NS_campaigns__funding_period > p') ||
					ele.querySelector('div.col.col-4.js-rewards-column.sticky-rewards > div > div.NS_campaigns__funding_period > p');
				return {
...
//					duration: ele.textContent?.match(/\(.*\)/)[0]?.split(' ')[0]?.slice(1) *1
		...
		let shownNumberOfComments;

		if(countingDataPot !== null) {
			shownNumberOfFAQ = await page.evaluate(ele => JSON.parse(ele.getAttribute('data-campaign')).projectFAQsCount, countingDataPot);
			shownNumberOfUpdates = await page.evaluate(ele => JSON.parse(ele.getAttribute('data-campaign')).updateCount, countingDataPot);
			...
			for(each of dataPot){
				...
				if(await page.evaluate(ele => ele.getAttribute('data-analytics'), each) === 'updates') shownNumberOfUpdates = await page.evaluate(ele => ele.getAttribute('emoji-data'), each)
				if(await page.evaluate(ele => ele.getAttribute('data-analytics'), each) === 'comments') shownNumberOfComments = await page.evaluate(ele => ele.getAttribute('data-comments-count'), each)
			}
		}
...
const getCampaignData = async (url, slug) => {
	const browser = await puppeteer.launch(globalVariable.browserOptions);
	const page = await browser.newPage();     
	await page.setUserAgent(userAgent.random().toString());
	await page.setBypassCSP(true)
	...
	try {
		...
		await page.on('response', async response => {
			...
			if(response.url().includes('/graph')) {
				...
				if(data === undefined || data?.data?.project?.story === undefined) return
				else result = data.data.project;
			}
		})
